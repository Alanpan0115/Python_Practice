...
def a15_function_4(a):
    for i in range (0,a) :
        spaces = a-i
        stars = 2 * i - 1 #舉例當我要三層，由上到下他會是1、3、5 ，因此第一層他就是2*1-1 ，第二層他就是2*2-1以此類推
        print(' ' * spaces + '*' * stars)
a15_function_4(10)
# ----------------------------------------------------------------
# 16. 計算階乘
...
def a17_function(a):
    ...
    for i in a:
        if i % 2 ==0:
           ans += i
    print('Q17.',ans) 
a17 = [1,2,3,5,6,9,1,2,3,4,6,5,7,23,6,2,1,3,66,42,1,3,94,631,8,3,315]
a17_function(a17)
...
def a18_function(a):
    a18_avg = sum(a18)/len(a18)
    a18_ans = []
    for i in a:
        if i > a18_avg:
            a18_ans.append(i)
    print('Q18.',a18_ans,a18_avg)
        
a18 = [1,2,6,9,1,3,5,6,45,81,23,5,6,74,3,21,3,64,2,32,44,89,16,6,32,5,1,24,91,3,22,51,30,11,15,]
a18_function(a18)


# ----------------------------------------------------------------
# 19.移除列表中為負數的元素
# 寫一個函式，輸入一個整數列表，回傳一個新的列表，移除所有小於 0 的數字。
def a19_function(a):
    a19_ans = []
    for i in a :
        if i > 0:
            a19_ans.append(i)
    print('Q19.',a19_ans)  
a19=[12,-5,53,9,-56,16,-8,6,-9,-8,-12,53,41]
a19_function(a19)
# ----------------------------------------------------------------
# 20.統一字串格式為句首大寫
# 寫一個函式，輸入一段句子，將每個句子的第一個字母轉為大寫，其他字母轉為小寫。
def a20_function(a):
    import re
    # 用正則表示式分割句子，保留標點符號（. ? !）
    ...
    for i in range(0, len(sentences) - 1, 2):
        sentence = sentences[i].strip().lower().capitalize()
        punctuation = sentences[i+1]
        result += sentence + punctuation + " "

    print("Q20. " + result.strip())

# 測試
a20 = 'hello , my name is jon . what is your name ? '
a20_function(a20)
